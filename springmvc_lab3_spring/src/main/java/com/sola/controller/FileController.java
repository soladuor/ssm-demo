...
public class FileController {
    ...
    public JSONResult fileUpload(
        ...
        try {
            // 推荐的方法：创建自己的sessionContext，然后建立session监听实现HttpSessionListener接口
            MySessionContext myc = MySessionContext.getInstance();
            HttpSession userSession = myc.getSession(myAuth);
            // 获取当前用户信息
            FileUser user = (FileUser) userSession.getAttribute("user");
